...
const DB_NAME: string = AppConstant.localDbName;
...
export class Sql {
  ...
  constructor(public platform: Platform) {
    this._dbPromise = new Promise((resolve, reject) => {
      // try {
        ...
        .catch (err => {
          console.log('Plateform Not ready :', err);
          reject({err: err});
        });
      // } catch (err) {
      //   console.log('try db connection error: ', err);
      //   reject({err: err});
      // }
    });
    this._tryInit();
  ...
  query(query: string, params: any[] = []): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      // } catch (err) {
      //   console.log('Error on Read table :', err);
      //   reject({err: err});
      // }
    });
  }
